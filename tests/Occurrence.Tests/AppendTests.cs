...
namespace Occurrence.Tests
{
    public abstract class AppendTests : EventStoreTestBase
    {
        ...
        public async Task Should_Return_New_Current_Stream_Version(int countAndExpectedVersion)
        {
            ...
            var appendResult = await Subject.Append("1", eventDatas, 0);
        ...
        public async Task Should_Append_Events()
        {
            ...
            await Subject.Append("1", new[] { ed1, ed2 }, 0);
        ...
        {
            var exception = await Assert.ThrowsAsync<OptimisticConcurrencyException>(() => Subject.Append("1", new[] { new EventData(new TestEvent(), DateTime.UtcNow) }, 1));
